...
export const QueueDeployRequestInput = Schema.Struct({
  ...
  [ApiPath]: (input: { organization: string; database: string; number: string }) =>
    `/organizations/${input.organization}/databases/${input.database}/deploy-requests/${input.number}/deploy`,
...
export const queueDeployRequest = /*@__PURE__*/ /*#__PURE__*/ API.make(() => ({
  ...
  errors: [QueueDeployRequestUnauthorized, QueueDeployRequestForbidden, QueueDeployRequestNotfound],
}));
